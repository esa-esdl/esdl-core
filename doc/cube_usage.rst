...
The current version (version 1.0, Nov 2015) of the Data Cube is not explicitly gap-filled. However, a certain
gap-filling occurs during temporal aggregation as described in the next section. The CAB-LAB team may provide
gap-filled Data Cube versions at a later point in time of the project. Gap-filling is part of the *Data Analytics
...
resulting Data Cube target period. If there is more than one contribution in time, then we weight each contribution
...
Spatial resampling applies to an image that is the result of a former temporal resampling if the Data Cube's spatial
resolution is different from the data source resolution.

If the Data Cube's spatial resolution is higher than the data source spatial resolution, source images are **upsampled
by rescaling hereby duplicating original values, but not performing any spatial interpolation**.

If the Data Cube's spatial resolution is lower than the data source spatial resolution, source images are **downsampled
